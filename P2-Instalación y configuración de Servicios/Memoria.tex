...
\begin{enumerate}
	...
	\section{Instalación del servicio de acceso remoto a la consola(\textit{Secure Shell})}
		...
		\item En muchas ocasiones es necesario reiniciar un servicio para que tome los cambios
		en su configuración. Indique los comandos que puede emplear en Ubuntu y CentOS para
		hacerlo.
		
		Basándonos en \cite{cyberciti1} y \cite{cyberciti2}, podemos ver varias formas de
		reiniciar un servicio.
		
		Tomando como ejemplo el servicio SSH, en Fedora, CentOS o similares, se utilizaría
		\textit{/etc/init.d/sshd restart} o \textit{service sshd restart}.
		
		Mientras que en Debian, Ubuntu o similares, sería \textit{/etc/init.d/ssh restart}
		o \textit{service sshd restart}.
		
		También se puede hacer utilizando \textit{sudo restart ssh}.
