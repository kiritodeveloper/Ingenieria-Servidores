	...
	\subsection{Más Benchmarks\dots}
		...
		\begin{itemize}
			...
			\item \large\textbf{Experimento:}
				...
				Por ello, para el experimento, se utilizará mi disco duro, que contiene varias
				particiones, en concreto, la partición de \textit{Linux} con \textbf{ext4} y la
				partición de \textit{Windows} con \textbf{nfts} y una memoria externa, en concreto,
				un pendrive USB.
				
				La prueba consiste en realizar 5 veces:
				
				\begin{itemize}
					\item Tomar 1 MB del archivo pasado, y copiar hasta obtener 1GB\footnote{Nótese que
					si copiamos el contenido en un array auxiliar, concatenamos y repetimos, tras 1024
					...
					\item Para cada dirección pasada como parámetro:
						\begin{itemize}
							...
							\item Escribimos tantos Gigabytes como indique el parámetro pasado de uno
							en uno Gigabyte\footnote{Yo pasaré 5GB como tamaño}.
					
							\item Leemos tantos Gigabytes como indique el parámetro pasado de uno en
							uno Gigabyte.
					
							\item Leemos del primer archivo y escribimos en el segundo tantos
							Gigabytes como indique el parámetro pasado de uno en uno Gigabyte.
			...
			Los resultados se encuentran disponibles en los archivos adjuntos \textit{Resultados.csv}
			(sin ordenar) y \textit{Resultados.ods} (ya ordenados y analizados). Como podemos ver en
			la tabla:
			
			\begin{figure}[H]
				\centering
				\includegraphics[width=15cm]{Ejercicio_9.jpg}
